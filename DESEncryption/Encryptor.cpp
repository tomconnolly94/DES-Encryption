...
std::string Encryptor::Encrypt(std::string input) {

	//initial permutation
	input = ExecuteInitialPermutation(input);

	//split data into left and right parts
	std::vector<std::string> splitKey = Util::HalfVector(input);
	std::string leftPart = splitKey[0];
	std::string rightPart = splitKey[1];
}
