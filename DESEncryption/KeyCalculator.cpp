#include "KeyCalculator.h"
#include "Util.h"

KeyCalculator::KeyCalculator() {};

std::string KeyCalculator::InitalPermutation(std::string keyInput) {

	std::vector<int> keyIntialPermutationTable = {
		57, 49, 41, 33, 25, 17, 9,
		1, 58, 50, 42, 34, 26, 18,
		10, 2, 59, 51, 43, 35, 27,
		19, 11, 3, 60, 52, 44, 36,
		63, 55, 47, 39, 31, 23, 15,
		7, 62, 54, 46, 38, 30, 22,
		14, 6, 61, 53, 45, 37, 29,
		21, 13, 5, 28, 20, 12, 4
	};

	return Util::ExecutePermutation(keyInput, keyIntialPermutationTable);
}


std::vector<std::string> KeyCalculator::CalculateRoundKeys(std::string keyInput) {
	
	std::vector<std::string> splitKey = Util::HalfVector(keyInput);
	std::string leftKey = splitKey[0];
	std::string rightKey = splitKey[1];

	// key shift table
	int keyShiftTable[16] = {
		1, 1, 2, 2,
		2, 2, 2, 2,
		1, 2, 2, 2,
		2, 2, 2, 1
	};

	// key compression table 
	std::vector<int> keyCompressionPermutationTable = {
		14, 17, 11, 24, 1, 5,
		3, 28, 15, 6, 21, 10,
		23, 19, 12, 4, 26, 8,
		16, 7, 27, 20, 13, 2,
		41, 52, 31, 37, 47, 55,
		30, 40, 51, 45, 33, 48,
		44, 49, 39, 56, 34, 53,
		46, 42, 50, 36, 29, 32
	};

	std::vector<std::string> outputKeys = {};

	//16 rounds of encryption so 16 keys are required
	for (int roundIndex = 0; roundIndex < 16; ++roundIndex) {
		leftKey = Util::BitShiftLeft(leftKey, keyShiftTable[roundIndex]);
		rightKey = Util::BitShiftLeft(rightKey, keyShiftTable[roundIndex]);

		std::string key = leftKey + rightKey;

		//perform compression permutation
		key = Util::ExecutePermutation(key, keyCompressionPermutationTable);

		outputKeys.push_back(key);

	}
	return outputKeys;
}



